...
def get_db_connection():
    conn = sqlite3.connect('hierarchical_queue_roles.db')
...
def center_dashboard():
    ...
    return render_template_string('''
        ...
        <style>
            body { background: #0f172a; color: #fff; font-family: Tahoma; padding: 20px; margin: 0; }[cite: 5]
            .container { max-width: 1000px; margin: auto; }
            .card { background: #1e293b; padding: 20px; border-radius: 12px; margin-bottom: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.3); }[cite: 5]
            ...
            table { width: 100%; border-collapse: collapse; margin-top: 10px; background: #0f172a; }[cite: 5]
            th, td { padding: 12px; text-align: center; border-bottom: 1px solid #334155; font-size: 14px; } th { color: #f59e0b; }[cite: 5]
...
def kiosk_machine(center_id):
    ...
    return render_template_string('''
        ...
        <style>
            ...
            /* نافذة المعاينة المنبثقة (Modal) والتذكرة */
            ...
            /* إبراز المصلحة أو الشباك بشكل مميز وواضح */
            .service-highlight {
                background: #f0fdf4;
        ...
        <!-- نافذة معاينة التذكرة قبل الطباعة -->
        <div id="ticketModal" class="modal">
            <div class="ticket-card">
                <h2>{{ center.center_name }}</h2>
                
                <!-- اسم الشباك / الخدمة بارز بوضوح هنا -->
        ...
        <script>
            function issueTicket(sId){
                ...
                fetch('/api/issue-ticket/' + sId, {
                    ...
                    if(data.success){
                        // تعبئة بيانات التذكرة في نافذة المعاينة بدقة
...
def api_issue_ticket(service_id):
    ...
    # تنفيذ الإدراج والحصول على معرف التذكرة الجديد (lastrowid)
    cursor = conn.execute(
...
def employee_window():
    ...
    return render_template_string('''
        ...
        <style>
            body { background: #0f172a; color: #fff; font-family: Tahoma; padding: 20px; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; }[cite: 5]
...
def api_call_next(service_id):
    conn = get_db_connection()
    # التعديل الهام هنا: ترتيب جلب التذكرة التالية يعطي الأولوية المطلقة لذوي الاحتياجات الخاصة (is_priority DESC) ثم الأقدم تاريخاً (id ASC)
    next_t = conn.execute('''
...
def system_all_displays():
    ...
    return render_template_string('''
        ...
        <style>
            ...
            .center-card { background: #1e293b; border-radius: 12px; padding: 15px; border: 1px solid #334155; box-shadow: 0 4px 15px rgba(0,0,0,0.3); }[cite: 5]
...
def reset_tickets(center_id):
    ...
    reset_type = data.get('type', 'daily') # الافتراضي هو التصفير اليومي
    
    conn = get_db_connection()
    try:
        if reset_type == 'full':
            # تصفير كلي لجميع تذاكر المركز (مرحلة التهيئة والبداية)
            conn.execute('DELETE FROM queue_tokens WHERE center_id = ?', (center_id,))
            message = "تم تنفيذ التصفير الكلي لجميع تذاكر المركز بنجاح."
        else:
            # تصفير تذاكر اليوم الحالي فقط (الاستخدام اليومي)
            today_str = datetime.now().strftime('%Y-%m-%d')
            conn.execute('''
